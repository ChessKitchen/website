        ...
        <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled
            ...
            <nav className="container-brand flex items-center justify-between h-20">
                ...
                <div className="flex items-center">
                    <a href="/" className="flex items-center space-x-3 group">
                        <div className="relative">
                            ...
                            <div className="w-10 h-10 bg-gradient-to-br from-brand-500 via-brand-600 to-accent-royal-600 rounded-2xl flex items-center justify-center text-white font-bold text-xl group-hover:scale-110 transition-all duration-300 shadow-brand">
                            ...
                            {/* Glowing effect */}
                            <div className="absolute -inset-1 bg-gradient-to-br from-brand-500 to-accent-royal-600 rounded-2xl opacity-0 group-hover:opacity-30 transition-opacity duration-300 blur-md -z-10"></div>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-xl font-brand font-bold bg-gradient-to-r from-text-primary via-brand-600 to-accent-royal-600 bg-clip-text text-transparent">
                                Pawn Appétit
                            </span>
                        </div>
                    </a>
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <button
                        onClick={toggleTheme}
                        className="w-11 h-11 rounded-2xl bg-surface-secondary hover:bg-surface-tertiary border border-gray-200 dark:border-gray-700 flex items-center justify-center transition-all duration-300 group shadow-soft hover:shadow-medium"
                        aria-label="Toggle theme"
                    >
                        {theme === 'dark' ? (
                            <Sun className="h-5 w-5 text-accent-honey-500 group-hover:rotate-12 group-hover:scale-110 transition-transform duration-300" />
                        ) : (
                            <Moon className="h-5 w-5 text-accent-royal-600 group-hover:-rotate-12 group-hover:scale-110 transition-transform duration-300" />
                        )}
                    </button>
                    ...
                    >
                        <MessageCircle className="h-5 w-5 text-text-secondary group-hover:text-text-primary group-hover:scale-110 transition-all duration-300" />
                    </a>
                    ...
                    >
                        <Github className="h-5 w-5 text-text-secondary group-hover:text-text-primary group-hover:scale-110 transition-all duration-300" />
                    </a>

                    {/* Enhanced Mobile Menu Button */}
                    <button
                        onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                        className="lg:hidden w-11 h-11 rounded-2xl bg-surface-secondary hover:bg-surface-tertiary border border-gray-200 dark:border-gray-700 flex items-center justify-center transition-all duration-300 shadow-soft"
            ...
            <div className={`lg:hidden transition-all duration-500 overflow-hidden ${mobileMenuOpen
                ...
                <div className="bg-surface-primary/95 backdrop-blur-xl border-t border-gray-200/50 dark:border-gray-700/50 px-6 py-8 space-y-2">
                    {navigation.map((item, index) => (
                        <a
                            key={item.name}
                            href={item.href}
                            className="block px-4 py-4 text-base font-medium text-text-secondary hover:text-text-primary hover:bg-surface-secondary rounded-2xl transition-all duration-300 animate-fade-in-up"
